...
//Fetching the editing data from db//
	$pQuery = tep_db_query("select * from " . TABLE_PURCHASES . " where purchases_id = '" . $purId . "'");
	$pRow = tep_db_fetch_array($pQuery);
	$prQuery = tep_db_query("select * from " . TABLE_PURCHASES_REMARKS . " where purchases_id = '" . $purId . "'");
	$prRow = tep_db_fetch_array($prQuery);
	
//Fetching the editing data from db//
	
	$action = (isset($HTTP_GET_VARS['action']) ? $HTTP_GET_VARS['action'] : '');
	
				$parameters = array('purchases_id' => $pRow['purchases_id'],
									'products_name' => '',
			                        'products_id' => $pRow['products_id'],
  								    'purchase_date_of_input' => '',
					    			'purchase_date' => $pRow['purchase_date'],
								    'price_per_piece' => $pRow['price_per_piece'],
								    'purchase_quantity' => $pRow['purchase_quantity'],
					    			'shipping_cost' => $pRow['shipping_cost'],
								    'total_cost' => $pRow['total_cost'],
									'average_total_cost' => $pRow['average_total_cost'],
            			            'manufacturers_id' => $pRow['manufacturers_id'],
					    			'purchase_remarks' => $prRow['purchase_remarks']);
    	...
    	switch ($action) {
			case 'update':
			    ...
			    tep_db_perform(TABLE_PURCHASES, $sql_data_array, 'update', "purchases_id = '" . (int)$purId . "'");
	            ...
	            $insert_sql_data = array('purchases_id' => (int)$purId);                                     

            	$purchases_remarks_data_array = array_merge($purchases_remarks_data_array, $insert_sql_data);

            	tep_db_perform(TABLE_PURCHASES_REMARKS, $purchases_remarks_data_array, 'update', "purchases_id = '" . (int)$purId . "'");
				...
				} elseif ($pRow['purchase_quantity'] > $purchase_quantity) {
					$decrease_qty = (int)$pRow['purchase_quantity'] - (int)$purchase_quantity;
					$total_quantity = (int)$products_quantity['products_quantity'] - (int)$decrease_qty;
					tep_db_query("update " . TABLE_PRODUCTS . " set products_quantity = '" . (int)$total_quantity . "' where products_id = '" . (int)$products_id . "'");
				} elseif ($pRow['purchase_quantity'] < $purchase_quantity) {
					$increase_qty = (int)$purchase_quantity - (int)$pRow['purchase_quantity'];
					$total_quantity = (int)$products_quantity['products_quantity'] + (int)$increase_qty;
					tep_db_query("update " . TABLE_PRODUCTS . " set products_quantity = '" . (int)$total_quantity . "' where products_id = '" . (int)$products_id . "'");
				}
...
function updateTotal() {
  var pppValue = parseFloat(document.forms["editpurchase"].price_per_piece.value);
  var pqtyValue = parseFloat(document.forms["editpurchase"].purchase_quantity.value);
  var shpcstValue = parseFloat(document.forms["editpurchase"].shipping_cost.value);  
  var totalValue = parseFloat(pppValue * pqtyValue + shpcstValue);
  var avgtotalValue = parseFloat(totalValue / pqtyValue);

  document.forms["editpurchase"].total_cost.value = doRound(totalValue, 2);
  document.forms["editpurchase"].average_total_cost.value = doRound(avgtotalValue, 2);

}

//--></script>

<?php echo tep_draw_form('editpurchase', FILENAME_EDIT_PURCHASE, (isset($HTTP_GET_VARS['purId']) ? '&purId=' . $HTTP_GET_VARS['purId'] : '') . '&action=update', 'post', 'enctype="multipart/form-data"'); ?>
...
//--></script>
				...
				<tr>
					...
					<td><table border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td class="main"><?php echo tep_draw_textarea_field('purchase_remarks', 'soft', '70', '15', (empty($pInfo->purchases_id) ? '' : tep_get_purchases_remarks($pInfo->purchases_id))); ?></td>
						</tr>
					</table></td>
				</tr>
			</table>
		...
		<tr>
			<td class="smallText" align="right"><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_VIEW_PURCHASES)); ?></td>
		</tr>
	</table>
<script type="text/javascript">
